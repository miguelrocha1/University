...
void schedulerNew(Scheduler* scheduler, int programNumber){
    ...
    Process* process = createProcess(scheduler->numProcesses + 1, scheduler->instructions[programNumber - 1], scheduler->nInstructionsProgram);
...
void timeDecrementRunning(Scheduler* scheduler){
    if (scheduler->runningProcess != NULL){
        ...
        if(scheduler->runningProcess->time == 0){
